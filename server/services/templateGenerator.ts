 ...
 */
export async function generateTemplateWithLearning(
  category: 'text' | 'image' | 'video' | 'engineering',
  count: number = 3,
  successfulTemplates: any[] = []
): Promise<PromptTemplate[]> {
  if (!GEMINI_API_KEY) {
    throw new Error('Gemini API 키가 설정되지 않았습니다.')
  }
...
성공 템플릿 예시:
${learningContext.map((t: any, idx: number) => `
템플릿 ${idx + 1} (평점: ${t.rating}, 사용 횟수: ${t.usageCount}):
- 제목: ${t.structure.title}
- 설명: ${t.structure.description}
- 변수: ${t.variables.join(', ')}
- 섹션 구조:
${t.structure.sections.map((s: any) => `  - ${s.title}: ${s.content?.substring(0, 100)}...`).join('\n')}
`).join('\n')}
