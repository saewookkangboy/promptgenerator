...
node_cron_1.default.schedule('0 2 * * *', async () => {
    logger_1.log.info({ type: 'template_scheduler', task: 'usage_analysis' }, '템플릿 사용 패턴 분석 시작...');
    try {
        const templates = await prisma_1.prisma.template.findMany({
            where: { isPublic: true },
        });
        logger_1.log.debug({ type: 'template_scheduler', templateCount: templates.length }, '분석 대상 템플릿 수');
        let analyzedCount = 0;
...
node_cron_1.default.schedule('0 3 * * 1', async () => {
    logger_1.log.info({ type: 'template_scheduler', task: 'quality_evaluation' }, '템플릿 품질 평가 시작...');
    try {
        const templates = await prisma_1.prisma.template.findMany({
            where: { isPublic: true },
        });
...
node_cron_1.default.schedule('0 4 * * 2', async () => {
    logger_1.log.info({ type: 'template_scheduler', task: 'auto_generation' }, 'AI 템플릿 자동 생성 시작...');
    try {
        ...
        if (!admin) {
            logger_1.log.error({ type: 'template_scheduler', adminEmail }, 'Admin 사용자를 찾을 수 없습니다.');
            return;
        }
        logger_1.log.debug({ type: 'template_scheduler', adminId: admin.id }, 'Admin 사용자 확인 완료');
        ...
        for (const category of categories) {
            try {
                logger_1.log.debug({ type: 'template_scheduler', category, count: counts[category] }, '카테고리별 템플릿 생성 시작');
                // 기존 성공 템플릿 학습 데이터 수집
                const successfulTemplates = await prisma_1.prisma.template.findMany({
                    where: {
                        category: category,
                        isPublic: true,
                ...
                for (const template of templates) {
                    try {
                        ...
                        // 기존 템플릿 확인
                        const existing = await prisma_1.prisma.template.findFirst({
                            where: {
                                name: templateName,
                                category: category,
                            },
...
logger_1.log.info({ type: 'template_scheduler' }, '템플릿 스케줄러 초기화 완료');
