...
function generateRefreshToken(userId, email) {
    return jsonwebtoken_1.default.sign({ userId, email, type: 'refresh' }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' });
}
...
router.post('/login', validation_1.validateLoginInput, async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            logger_1.log.security('login_failed', {
        ...
        const refreshToken = generateRefreshToken(user.id, user.email);
        // 보안 이벤트: 로그인 성공
        logger_1.log.security('login_success', {
            userId: user.id,
            email: user.email,
            ip: req.ip,
            userAgent: req.get('user-agent'),
